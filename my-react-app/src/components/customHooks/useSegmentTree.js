...
const useSegmentTree = (pricesArray, param1, param2) =>{
  ...
  useEffect(() => {
    const checkInterval = () => {
      if(param1.current.val !== from){ setFrom(param1.current.value);}
      if(param2.current.valu !== to){ setTo(param2.current.value);}
    };
  ...
  return {rsi, cumulativeSum, intervalAverage, intervalVariance, aroonUp, aroonDown};
}
