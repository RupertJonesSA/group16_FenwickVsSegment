...
const disableTime = (dateTime, view) => {
  if (view === 'hours') {
    return !availableTimeSlots[dateTime.format('YYYY-MM-DD')]?.some(slot => 
      slot.hour === dateTime.hour()
    );
  }
  if (view === 'minutes') {
    const currentHourSlots = availableTimeSlots[dateTime.format('YYYY-MM-DD')]?.filter(slot => 
      slot.hour === dateTime.hour()
    );
    return !currentHourSlots?.some(slot => 
...
const calcIndex = (range) => {
  ...
  return -1;
    ...
    const daysAllowed = () => {
      ...
      const availableDates = tempData.map((i) => (JSON.stringify(i)).substring(1,11));
    ...
    const timeAllowed = () => {
      ...
      const dateStrings = timeArr.map((timestamp) => new Date(timestamp * 1000).toISOString());
    ...
    const availableTimeSlots = timeAllowed();
      ...
      return (
        <div>
          <button onClick={()=>setIsOn(!isOn)} className="bg-white rounded-lg w-[300px] h-20 text-3xl font-semibold text-gray-400 mt-10 mb-10 hover:text-black">Custom Range {isOn ? <span className="text-3xl">&#x25B2;</span> : <span className="text-2xl">&#x25BC;</span>}</button>
          {isOn ? <div className="flex flex-col justify-center gap-5 text-center text-2xl font-semibold text-white items-center">
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
              ...
              <DemoContainer components={['DatePicker', 'DateTimePicker']}>
                ...
                <div className="flex flex-row gap-3">
                  <DatePicker label="Lower Bound" sx={style} onChange={handleDateChange1} minDate={dayjs(calcMin())} maxDate={dayjs(calcMax())} shouldDisableDate={(date) => !daysAllowed().some(availableDate => date.isSame(dayjs(availableDate), 'day'))}/>
                  <DatePicker label="Upper Bound" sx={style} onChange={handleDateChange2} minDate={dayjs(calcMin())} maxDate={dayjs(calcMax())} shouldDisableDate={(date) => !daysAllowed().some(availableDate => date.isSame(dayjs(availableDate), 'day'))}/>
                </div>
                : interval === "weekly" ? <div className="flex flex-row gap-3">
                <DatePicker label="Lower Bound" onChange={handleDateChange1} sx={style} minDate={dayjs(calcMin())} maxDate={dayjs(calcMax())} shouldDisableDate={(date) => !daysAllowed().some(availableDate => date.isSame(dayjs(availableDate), 'day'))}/>
                <DatePicker label="Upper Bound" onChange={handleDateChange2} sx={style} minDate={dayjs(calcMin())} maxDate={dayjs(calcMax())} shouldDisableDate={(date) => !daysAllowed().some(availableDate => date.isSame(dayjs(availableDate), 'day'))}/>
                </div>
                :
                interval === "monthly" ?
                <Grid container justifyContent="center" alignItems="center">
                    <Grid item>
                      <div className="flex flex-row gap-3">
                      <DatePicker views={['year', 'month']} label="Lower Bound" format="YYYY-MM" onChange={handleDateChange1} sx={style} minDate={dayjs(calcMinMonthly())} maxDate={dayjs(calcMaxMonthly())}/>
                      <DatePicker views={['year', 'month']} label="Upper Bound" format="YYYY-MM" onChange={handleDateChange2} sx={style} minDate={dayjs(calcMinMonthly())} maxDate={dayjs(calcMaxMonthly())}/>
                      </div>
                    </Grid>
                  </Grid>
                  :
                  interval === "intraday" ?
                  <div className="flex flex-row gap-3">
                  <DateTimePicker label="Lower Bound" sx={style} onChange={handleDateChange1} minDate={dayjs(calcMin())} maxDate={dayjs(calcMax())} shouldDisableTime={disableTime} shouldDisableDate={(date) => !daysAllowed().some(availableDate => date.isSame(dayjs(availableDate), 'day'))}/>
                  <DateTimePicker label="Upper Bound" sx={style} onChange={handleDateChange2} minDate={dayjs(calcMin())} maxDate={dayjs(calcMax())} shouldDisableTime={disableTime} shouldDisableDate={(date) => !daysAllowed().some(availableDate => date.isSame(dayjs(availableDate), 'day'))}/>
                </div>
                : null
                }
              </DemoContainer>
          ...
          <div className="flex justify-center mb-10 flex-col gap-7">
            <button onClick={() => setIndexArr(handleSubmit)} className="font-normal bg-white text-gray-400 w-[200px] h-[50px] rounded-lg hover:text-black">Calculate</button>
            <button onClick={() => setIndexArr([0, timeArr.length-1])} className="text-gray-400 text-sm underline hover:text-white">Reset Bounds</button>
          </div>
          </div>
          : null}
      </div>
